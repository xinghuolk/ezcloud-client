...
export const serialApi = {
  ...
  // 导出序列号Excel - 使用原始axios以保留headers
  exportSerials: async (batch_id: string): Promise<{ blob: Blob; filename: string }> => {
    // 直接使用axios而不是request，以避免拦截器处理
    const axios = (await import('axios')).default
    const token = localStorage.getItem('token')
    
    // 获取API基础URL（与request.ts保持一致）
    const baseURL = import.meta.env.VITE_API_BASE_URL || '/api/v1'
    const url = `${baseURL}/serials/export`
    ...
    let filename = `serials_${batch_id}.xlsx` // 默认文件名
...
export const firmwareApi = {
  ...
  ): Promise<ApiResponse<FirmwareUploadResponse>> => {
