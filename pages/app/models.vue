...
const getDeviceTypeColor = (type: string): VTagColor => {
  // 使用统一的 info 颜色，与 Vendor Name 保持一致
  return 'info'
}
...
const validateForm = () => {
  ...
  if (!form.serial_alias.trim()) {
    errors.value.serial_alias = 'Serial alias is required'
    isValid = false
  } else if (!/^[A-Z0-9]{3}$/.test(form.serial_alias)) {
    errors.value.serial_alias = 'Serial alias must be exactly 3 alphanumeric characters'
    isValid = false
  }
...
const handleEdit = async (model: DeviceModel) => {
  ...
  Object.assign(form, {
    ...
    serial_alias: model.serial_alias || '',
        ...
        <template #default="wrapperState">
          ...
          <VFlexTable rounded>
            ...
            <template #body-cell="{ row: model, column }">
              ...
              <template v-if="column.key === 'serial_alias'">
                <VTag color="primary" rounded>
                  {{ model.serial_alias || '-' }}
                </VTag>
              </template>
    ...
    >
      <template #content>
        <form @submit.prevent="handleSubmit">
          ...
          <VField>
            <VLabel>Serial Alias *</VLabel>
            <VControl>
              <VInput
                v-model="form.serial_alias"
                placeholder="3 characters (A-Z, 0-9)"
                maxlength="3"
                @input="form.serial_alias = form.serial_alias.toUpperCase().replace(/[^A-Z0-9]/g, '')"
                :class="{ 'is-danger': errors.serial_alias }"
              />
              <p class="help">Must be exactly 3 alphanumeric characters (e.g., A01, B23, XYZ)</p>
              <p v-if="errors.serial_alias" class="help is-danger">
                {{ errors.serial_alias }}
              </p>
            </VControl>
          </VField>
      ...
      <template #content>
        <div v-if="currentModel" class="model-details">
          <div class="common-info-grid">
            ...
            <div class="common-info-item">
              <label>Serial Alias</label>
              <VTag color="primary">{{ currentModel.serial_alias || '-' }}</VTag>
            </div>
            <div class="common-info-item">
