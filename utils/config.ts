 ...
 */
export function getConfig(): EzenCloudConfig {
 ...
 */
export function getConfigValue(key: keyof EzenCloudConfig): string | undefined {
  const configStore = getConfigStore()
 ...
 * 同步获取特定配置项（推荐使用方式）
 */
export function getConfigValueSync(key: keyof EzenCloudConfig): string | undefined {
...
declare global {
  interface Window {
    __EZENCLOUD_CONFIG__?: EzenCloudConfig
  }
}